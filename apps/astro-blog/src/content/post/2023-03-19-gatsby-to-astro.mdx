...
featuredImage: /20230319/cover.webp
...
<PostImage
  src={import('../../assets/postImages/20230319/cover.webp')}
  alt="Astro의 성능 측정 결과"
/>
...
<PostImage
  src={import('../../assets/postImages/20230319/island-architecture.webp')}
  alt="출처: Islands Architecture: Jason Miller"
/>
...
<PostImage
  src={import('../../assets/postImages/20230319/ratios_over_time.webp')}
  alt="State Of JS 2022 Rendering Frameworks"
/>
...
<PostImage
  src={import('../../assets/postImages/20230319/astro-island.webp')}
  alt="preact로 개발된 TOC컴포넌트의 SSR 결과"
/>
...
<PostImage
  src={import('../../assets/postImages/20230319/network.webp')}
  alt="동적으로 로드된 JS들. react 런타임마저도 동적으로 로드되고 있다."
/>
...
필요한 곳에 일일히 직접 구현해 적용해야 했던 과거와 달리 엄청 간편해진 셈이다.
...
<PostImage
  src={import('../../assets/postImages/20230319/blog-lighthouse.webp')}
  alt="astro로 옮긴 후 측정한 결과. 이전에도 좋았지만 훨씬 좋아졌다."
/>

이는 Astro로 개발한 이 블로그의 홈 페이지에서 이미지와 GA를 제외한 초기 사이즈 (약 30KB)대비 57.14%나 많은 수치이다. (이 블로그 뿐만 아니라 Astro공식 홈페이지의 Showcase에 있는 사이트들은 모두 매우 작은 크기를 자랑하고 있다)
...
이 블로그도 이전에 gatsby로 개발했었는데. "블로그" 본질적인 기능인 "글 제공" 만을 남긴다고 했을 때 불필요한 JS가 너무 많았다. 스타터 템플릿이 그런데 예전 gatsby버전은 더 했을 것이다.
...
반면에 대시보드 앱과 같이 인터렉션이 필요한 UI가 많은 상황에서는 페이지 로드와 동시에 모든 JS를 받아 처리애햐 할 것이다. 이렇게 되면 Astro의 모든 장점을 극대화하기가 어려워진다. 이럴 땐 핸들러 단위로 JS를 쪼개주는 [qwik](https://qwik.builder.io/)을 고려해보는 것이 좋겠다.
